<script>
    ...
    export let params = {};
    let question_id = params.question_id || 0;
    ...
    function get_question() {
        console.log(`디버깅: 질문 ID -> ${question_id}`);
        fastapi(
            "get",
            "/api/question/detail/" + question_id,
            {},
            (json) => {
                console.log("디버깅: 질문 데이터", json);
                question = json;
            },
            (err_json) => {
                console.error("질문 데이터 로드 실패:", err_json);
                error = err_json;
            }
        );
    }
    ...
    function post_answer(event) {
        ...
        fastapi("post", url, params, (json) => {
            ...
            console.error("답변 등록 실패:", err_json);
            error = err_json;
        });
    }
    ...
    function delete_question(_question_id) {
        if (window.confirm("정말로 삭제하시겠습니까?")) {
            ...
            fastapi("delete", url, params, (json) => {
                ...
                console.error("질문 삭제 실패:", err_json);
                error = err_json;
            });
        }
    }
    ...
    function delete_answer(answer_id) {
        if (window.confirm("정말로 삭제하시겠습니까?")) {
            ...
            fastapi("delete", url, params, (json) => {
                ...
                console.error("답변 삭제 실패:", err_json);
                error = err_json;
            });
        }
    }
    ...
    function vote_question(_question_id) {
        if (window.confirm("정말로 추천하시겠습니까?")) {
            ...
            fastapi("post", url, params, (json) => {
                ...
                console.error("질문 추천 실패:", err_json);
                error = err_json;
            });
        }
    }
    ...
    function vote_answer(answer_id) {
        if (window.confirm("정말로 추천하시겠습니까?")) {
            ...
            fastapi("post", url, params, (json) => {
                ...
                console.error("답변 추천 실패:", err_json);
                error = err_json;
            });
        }
    }
...
<div class="container my-3">
    <!-- 질문 -->
    <h2 class="border-bottom py-2">{question.subject || "제목 없음"}</h2>
    <div class="card my-3">
        <div class="card-body">
            <div class="card-text">
                {@html marked.parse(question.content || "내용 없음")}
            </div>
            <div class="d-flex justify-content-end">
                ...
                <div class="badge bg-light text-dark p-2 text-start">
                    <div class="mb-2">{question.user ? question.user.username : "익명"}</div>
                    <div>
    ...
    <div class="card my-3">
        <div class="card-body">
            <div class="card-text">
                {@html marked.parse(answer.content || "내용 없음")}
            </div>
            <div class="d-flex justify-content-end">
                ...
                {/if}
                <div class="badge bg-light text-dark p-2 text-start">
                    <div class="mb-2">{answer.user ? answer.user.username : "익명"}</div>
                    <div>
